...
import { fetchStockNames, fetchStockDetail } from './api';
        ...
        {stockDetailSuccess && (
          <section className="mx-10 flex flex-col md:flex-row md:items-center">
            <div className="w-full text-center md:text-left">
              ...
              <p
                className={`my-4 flex flex-row items-center justify-center gap-2 text-base md:justify-start ${
                  stockDetailData && stockDetailData?.ltp > stockDetailData?.close ? `text-green-400` : `text-red-400`
                }`}
              >
                Last Trading Price: {stockDetailData?.ltp}{' '}
                {stockDetailData && stockDetailData?.ltp > stockDetailData?.close ? (
                  <BsChevronDoubleUp className={`animate-bounce text-base text-green-400`} />
                ) : (
                  <BsChevronDoubleDown className={`animate-bounce text-base text-red-400`} />
                )}
              </p>
            ...
            <div className="w-full rounded-lg bg-white shadow dark:border dark:border-gray-700 dark:bg-gray-800 md:mt-0 xl:p-0">
              <div className="space-y-4 p-6 sm:p-8 md:space-y-6">
                ...
                <form className="space-y-4 md:space-y-6">
                  ...
                  <button className="rounded-lg bg-blue-600 px-5 py-2.5 text-center text-sm font-medium text-white hover:bg-blue-700 focus:outline-none focus:ring-4 focus:ring-blue-300 dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">
                    Buy
                  </button>{' '}
                  <button className="rounded-lg bg-red-600 px-5 py-2.5 text-center text-sm font-medium text-white hover:bg-red-700 focus:outline-none focus:ring-4 focus:ring-red-300 dark:bg-red-600 dark:hover:bg-red-700 dark:focus:ring-red-800">
                    Sell
                  </button>
                </form>
              </div>
